...
# Gate: block `git commit` while a required obligation is outstanding.
set -euo pipefail
exec "${KERNEL_BIN:-kernel}" pre-commit --ledger "evidence/events.jsonl" --require require-validation
